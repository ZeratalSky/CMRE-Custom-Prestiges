...
双击 `选择安装的威望.bat`，列表会显示所有可用威望及当前安装状态。可以先按指挥官筛选，再勾选需要保留的威望。点击“保存并同步”后，工具会安装勾选项，并自动卸载未勾选项。

首次使用且尚未保存选择时，默认勾选全部威望。以后仓库新增的威望会出现在列表中，但不会擅自改变已经保存的选择。
...
- 停用或重新启用单个威望：双击 `选择安装的威望.bat`，修改勾选后点击“保存并同步”。
